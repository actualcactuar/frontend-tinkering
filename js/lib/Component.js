...
export class Component extends HTMLElement {
  ...
  updateRender(change) {
    console.log(
      `%c🚀 Updating "${this.constructor.name}" component values "${Object.keys(
        change
      )}"`,
        ...
        for (const [attr, key] of Object.entries(childNode.dataset)) {
          if (this[key] || key in state.get()) {
            if (typeof this[key] === 'function') {
              // look for eventhandler in components class methods
              if (!memo.has(key)) {
              ...
              const value =
                typeof state.get(key) === 'function'
                  ? state.get(key).call(this)
                  : state.get(key);
            ...
            const data = keys.reduce(
              (acc, curr) => (acc && acc[curr]) || state.get(curr),
              null
            );
